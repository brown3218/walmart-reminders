...
export function applyReminderSnapshot(db: AppDatabase, snapshot: ReminderInput[]): ReminderSnapshotResult {
  ...
  const result: ReminderSnapshotResult = { created: 0, updated: 0, completed: 0, missing: 0 };
  ...
  for (const entry of snapshot) {
    ...
    else if (entry.completed && !existing.completed) result.completed += 1;
  ...
  for (const [externalId] of previousActive) {
    ...
    const item = db
      .listItems()
      .find((candidate) => candidate.external_id === externalId) as { id: number } | undefined;
    if (item) {
      db.deleteItem(Number(item.id), "reminder_snapshot");
      result.missing += 1;
    }
  }

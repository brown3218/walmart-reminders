...
`npm run doctor` checks Node, config presence, deprecated config keys, dashboard PIN safety, SQLite, Walmart session/manual-action status, Reminders helper status, Walmart profile directory, dashboard reachability, protected dashboard auth, and LAN URL detection.
